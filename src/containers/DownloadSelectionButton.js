...
const mapStateToProps = (state) => ({
  ...
  progress: (Object.keys(state.videos).map(id => state.videos[id]).filter(video => video.selected).map(video => video.progress).reduce((accumulator, progress) => accumulator + progress, 0) / Object.keys(state.videos).filter(id => state.videos[id].selected).length),
